...
import { getCredits, getMovie, getRelease } from '../../api/api';
...
export const FilmPage = () => {
  ...
  useEffect(() => {
    ...
    getCredits({ endpoint: `${id}/credits` }).then((data) => setCredits(data));
    getRelease({ endpoint: `${id}/release_dates` }).then((data) => setRelease(data));
  }, []);
            ...
            {activeLabel === 'genres' && (
              <>
                {movie?.genres.map((genre, index) => (
                  <LabelButton key={index} label={genre.name} />
                ))}
              </>
            )}
